...
ipcMain.on("calcul-score", async (event) => {
  // Récupère les chemins des 2 jsons
  const jsonPathReponse = path.join(folderData, "Test2Reponse.json");
  const jsonPathProtocole = path.join(folderData, "Test2.json");
  try {
    // Lis les fichiers des 2 jsons
    const jsonRawReponse = await fs.promises.readFile(jsonPathReponse, "utf-8");
    const jsonRawProtocole = await fs.promises.readFile(
      jsonPathProtocole,
      "utf-8",
    );
    // Transforme les json en objet
    const jsonObjectReponse = JSON.parse(jsonRawReponse);
    const jsonObjectProtocole = JSON.parse(jsonRawProtocole);

    // Initialisation des scores et d'un objet qui va contenir les réponses données
    let score = 0;
    const givenAnswer = {};

    // Parcours l'objet pour y mettre les réponses données
    jsonObjectReponse.forEach((item) =>
      item.questionsAudio.forEach((p) => {
        givenAnswer[p.id] = p.answer;
      }),
    );

    // Parcours l'objet protocole et compare les réponses données avec les réponses du protocole
    jsonObjectProtocole.forEach((item) => {
      item.questionsAudio.forEach((p) => {
...
ipcMain.on("calcul-shift", async (event) => {
  // Récupère les chemins des 2 jsons
  const jsonPathReponse1 = path.join(folderData, "Test2Reponse.json");
  const jsonPathReponse2 = path.join(folderData, "Test2Yield2.json");

  try {
    // Lis les 2 jsons
    const jsonRawReponse1 = await fs.promises.readFile(
      jsonPathReponse1,
      "utf-8",
    );
    const jsonRawReponse2 = await fs.promises.readFile(
      jsonPathReponse2,
      "utf-8",
    );

    // Transforme les json en objets
    const jsonObjectReponse1 = JSON.parse(jsonRawReponse1);
    const jsonObjectReponse2 = JSON.parse(jsonRawReponse2);

    // Initialisation du score et des réponses données
    let scoreShift = 0;
    const givenAnswer = {};

    // Parcours le yield1 et y attribue les réponses données
    jsonObjectReponse1.forEach((item) => {
      item.questionsAudio.forEach((p) => {
        givenAnswer[p.id] = p.answer;
      });
    });

    // Parcours le yield2, attribue les réponses données
    jsonObjectReponse2.forEach((item) => {
      item.questionsAudio.forEach((p) => {
        const answerYield2 = p.answer;
        // Compare les réponses entre le yield1 et le yield2, augmente le score si les réponses sont différentes
        if (givenAnswer[p.id] !== answerYield2) {
          scoreShift++;
        }
      });
    });
    console.log("scoreShift : ", scoreShift);
    return scoreShift;
  } catch (err) {
    console.error("Erreur lors du calcul du shift : ", err);
  }
});
